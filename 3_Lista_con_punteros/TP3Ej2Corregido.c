...
void insertarK(int k, tDato pDato)
{
	...
	if (!listaVacia(aux))
	{
		...
		printf("\nProducto agregado en posici�n %d.\n", k);
	}
	else
	{
...
void eliminarK(int k)
{
	...
	if (!listaVacia(aux))
	{
		...
		free(nodoSuprimir);
		nodoSuprimir = NULL;
		printf("\nSe elimin� producto en posici�n %d.\n", k);
	}
	else
	{
...
void preguntarIngresoProducto(tDato pDato)
{
	...
	printf("\nQuiere ingresar el producto en cualquier posici�n? (1.si/2.no): ");
	scanf("%d", &opcion);
	if (opcion == 1)
	{
		printf("\nDigite la posici�n donde quiere agregar el producto: ");
		scanf("%d", &posK);
...
void preguntarEliminarProducto()
{
	...
	printf("\nQuiere eliminar el producto de cualquier posici�n? (1.si/2.no): ");
	scanf("%d", &opcionE);

	if (opcionE == 1)
	{
		printf("\nDigite la posici�n a eliminar: ");
		scanf("%d", &posKE);
		eliminarK(posKE);
	}
	else
